...
from esami join corsi on esami.corso=corsi.id
						join corsidilaurea on corsi.corsodilaurea=corsidilaurea.id
group by corsidilaurea.denominazione, corsidilaurea.facolta -- la clausola group by è sempre necessaria nel caso di funzioni aggregate come max;
...
---esercizio5---
select corsi.denominazione, count(esami) as "Numero Esami"
from corsi
	left outer join esami on esami.corso=corsi.id
	left outer join corsidilaurea on corsi.corsodilaurea=corsidilaurea.id
where esami.data > '2012-01-04'
group by corsi.denominazione having count(*) = 0
union
select corsi.denominazione, count(*) as "Numero Esami"
from esami
	right outer join corsi on esami.corso=corsi.id
	left outer join corsidilaurea on corsi.corsodilaurea=corsidilaurea.id
where corsidilaurea.denominazione='Informatica' and esami.data > '2012-01-04'
group by corsi.denominazione having count(*)>5;/*  questa parte va bene */
